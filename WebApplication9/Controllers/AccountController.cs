        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var account = await UserManager.FindByNameAsync(model.Email);
                    bool isLogged = Guid.TryParse(account.Id, out var accountId);
                    if (isLogged)
                    {
                        ...
                        {
                            var admin = new Administrator() { AccountID = accountId };
                            _applicationDbContext.Administrators.Add(admin);
                        }
                        else if(model.Role == "Student")
                        {
                            var student = new Student() { AccountID = accountId, GroupID = 1 }; // nie zadziala
